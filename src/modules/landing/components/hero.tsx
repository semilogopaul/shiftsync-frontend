...
export function Hero() {
  const [isDesktop, setIsDesktop] = useState(true); // Default true for SSR hydration match if preferred, or handle cautiously
  ...
  return (
    <section
      ...
      className="relative isolate min-h-[100dvh] overflow-hidden flex flex-col items-center justify-center pt-24 pb-12"
    >
      <div
        aria-hidden="true"
      ...
      {/* Orbiting Avatars around the Text */}
        ...
        {AVATARS.map((avatar, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={cn(
                  "absolute -bottom-5 -right-5 drop-shadow-xl sm:block",
                  avatar.color
                )}
              >
      ...
      <div className="relative mx-auto w-full max-w-5xl px-6 lg:px-8 text-center flex flex-col items-center z-20 mt-10 sm:mt-0">
        <h1
          id="hero-heading"
          className="text-4xl sm:text-5xl md:text-7xl font-bold tracking-tight mb-8 max-w-4xl mx-auto leading-tight"
        >
        ...
        <p className="max-w-2xl text-base sm:text-xl text-muted-foreground mb-10 px-2 sm:px-0 leading-relaxed font-medium bg-background/50 sm:bg-transparent backdrop-blur-md sm:backdrop-blur-none rounded-2xl py-3 border border-border/50 sm:border-transparent">
          Bring your whole team together. Watch shifts fill instantly. Say goodbye to spreadsheet chaos and frantic group chats.
        </p>

        <div className="flex flex-col sm:flex-row gap-4 items-center w-full justify-center px-4 sm:px-0">
          <Button size="lg" asChild className="w-full sm:w-auto h-14 px-8 text-lg font-bold rounded-2xl shadow-lg hover:scale-105 transition-transform duration-300">
             <Link href="/register">Start for free</Link>
          </Button>
          <Button size="lg" variant="outline" asChild className="w-full sm:w-auto h-14 px-8 text-lg font-bold rounded-2xl bg-secondary/50 backdrop-blur-md border-border/50 hover:bg-secondary transition-colors duration-300">
