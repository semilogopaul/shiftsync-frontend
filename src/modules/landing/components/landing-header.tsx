  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 sm:px-6 lg:px-8 transition-all duration-300">
        ...
        <nav aria-label="Primary" className="hidden lg:flex items-center gap-1.5 bg-secondary/40 border border-border/50 rounded-full px-3 py-2 backdrop-blur-xl shadow-sm">
          {NAV_LINKS.map((link) => {
            const active =
              link.href === "/"
                ? pathname === "/"
            ...
            return (
              <Link
                ...
                href={link.href}
                className={cn(
                  "relative rounded-full px-5 py-2 text-[15px] font-semibold transition-all duration-300",
                  active
        ...
        <div className="flex items-center gap-3">
          ...
          <Button asChild className="rounded-full shadow-lg font-bold text-[15px] px-6 py-5 hidden md:inline-flex hover:-translate-y-0.5 transition-transform">
          ...
          <div className="relative w-[300px] h-full bg-secondary/80 backdrop-blur-2xl border-l border-border/40 shadow-2xl flex flex-col pt-8 px-6 animate-in slide-in-from-right">
            ...
            <nav aria-label="Mobile primary" className="flex flex-col space-y-4 w-full">
              ...
              <div className="mt-8 flex flex-col gap-4 border-t border-border/40 pt-8">
                <Button asChild variant="outline" size="lg" className="rounded-xl w-full h-14 text-lg font-bold">
                  <Link href="/login">Log in</Link>
                </Button>
                <Button asChild size="lg" className="rounded-xl w-full h-14 text-lg font-bold shadow-lg">
                  <Link href="/register">Sign up</Link>
                </Button>
              </div>
